...
class TvShowRepositoryImpl(
    ...
    override suspend fun getAll(): Flow<State<MoviePagination<TvShow>>> =
        object :
            NetworkBoundRepository<MoviePagination<TvShowResult>, MoviePagination<TvShow>>() {

            override suspend fun fetchFromRemote(): Response<MoviePagination<TvShowResult>> =
                tvShowRoutes.getTvShowsPopular()

            override suspend fun mapFromRemote(request: MoviePagination<TvShowResult>): MoviePagination<TvShow> {
    ...
    override suspend fun get(id: Long): Flow<State<TvShow?>> =
        object :
            NetworkBoundRepository<TvShowDetailResult, TvShow?>() {
            override suspend fun fetchFromRemote(): Response<TvShowDetailResult> =
                tvShowRoutes.getTvShow(id)

            override suspend fun mapFromRemote(request: TvShowDetailResult): TvShow? =
                request.mapToTvShow()
